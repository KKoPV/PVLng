...
/**
 *
 * Don't forget to define a database and a cache by
 * - ORMTable::setDB() and
 * - ORMTable::setCache()
 * before 1st call!
 *
...
abstract class ORMTable {
    ...
    public function __construct ( $id=NULL ) {
        ...
        if (!is_null($id)) $this->findPrimary($id);
    }
        ...
        foreach ($this->fields as $field=>$value) {
            ...
            if (!in_array($field, $this->primary) AND
                ...
                if ($value == '' AND $this->nullable[$field]) {
                    ...
                    $set[] = sprintf('%s = "%s"', $field, self::$db->real_escape_string($value));
                }
            }
        }
    ...
    protected function _where( $fields, $values ) {
        ...
        if (!empty($fields)) {
            ...
            foreach ($fields as $id=>$field) {
                ...
                $where[] = $field.' = '
                         . '"' . self::$db->real_escape_string($values[$id]) .'"';
            }
        }
