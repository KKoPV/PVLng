...
$api->get(
    ...
    function ($id) use ($api) {
        ...
        // Return without API key only public channels
        foreach ((new ORM\Tree)->getWithParents(!$api->APIKeyValid) as $channel) {
            if ($channel['id'] != $id && $channel['parent'] != $id) {
                continue;
            }
            unset($channel['entity']);
            $channels[] = $channel;
...
$api->put(
    '/tree/:pguid/:cguid',
    $APIkeyRequired,
    function ($pguid, $cguid) use ($api) {
        if ($pguid == '0000-0000-0000-0000-0000-0000-0000-0000') {
            // Interpret as "root node"
            $parent = Channel\Channel::byId(1);
        } else {
            $parent = Channel\Channel::byGUID($pguid);
        }

        $child = Channel\Channel::byGUID($cguid);
        $id = $parent->addChild($child->getId());

        // Recreate child to read all attributes
        $child = Channel\Channel::byGUID($id);
    }
)
->conditions(array(
    'pguid'       => '(\w{4}-){7}\w{4}',
...
$api->put(
    ...
    function ($id) use ($api) {
        ...
        if (!$channel->getId()) {
            $api->stopAPI('Unkown hierarchy Id: '.$id, 404);
        }
        if ($channel->getAlias()) {
            $api->stopAPI(I18N::translate('AliasStillExists'));
        }
        ...
        if (!$alias->isError()) {
            $api->stopAPI(I18N::translate('AliasCreated'), 200);
        } else {
            $api->stopAPI($alias->Error());
        }
    }
)
->name('PUT /tree/alias/:id')
->help = array(
    'since'       => 'r4',
...
$api->delete(
    ...
    $APIkeyRequired,
    function ($id) use ($api) {
        if (Channel\Channel::ById($id, false)->removeFromTree()) {
            $api->halt(204);
        } else {
