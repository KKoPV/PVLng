...
abstract class ORM implements \Iterator, \Countable
{
    ...
    public static function checkMemoryTable()
    {
        ...
        if (static::$memory) {
            ...
            self::$db->query(static::$createSQL);
            static::$memory = false;
        }
    }
    ...
    protected function doInsert($mode)
    {
        ...
        try {
            if ($this->runQuery($sql) && $this->autoinc && self::$db->insert_id) {
    ...
    protected function runQuery($sql)
    {
        ...
        if (self::$db->errno == 1149) {
            die(self::$db->error . ' : ' . $sql);
        }

        if ($this->throwException && self::$db->errno) {
