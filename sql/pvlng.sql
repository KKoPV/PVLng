...
/*!50001 CREATE TABLE `pvlng_babelkit_translate` (
  `code_set` tinyint NOT NULL,
  `code_code` tinyint NOT NULL,
  `code_desc` tinyint NOT NULL
) ENGINE=MyISAM */;
...
CREATE TABLE `pvlng_tree` (
  ...
  KEY `lft_rgt` (`lft`,`rgt`),
  KEY `guid` (`guid`),
  CONSTRAINT `pvlng_tree_ibfk_1` FOREIGN KEY (`entity`) REFERENCES `pvlng_channel` (`id`)
...
CREATE TABLE `pvlng_view` (
  ...
  `public` tinyint(1) unsigned NOT NULL COMMENT 'Public view',
...
/*!40101 SET character_set_client = @saved_cs_client */;
/*!50001 DROP TABLE IF EXISTS `pvlng_babelkit_translate`*/;
...
/*!50001 CREATE  */
/*!50013   */
/*!50001 VIEW `pvlng_babelkit_translate` AS select `pvlng_babelkit`.`code_set` AS `code_set`,`pvlng_babelkit`.`code_code` AS `code_code`,`pvlng_babelkit`.`code_desc` AS `code_desc` from `pvlng_babelkit` where ((not((`pvlng_babelkit`.`code_set` like 'code%'))) and (`pvlng_babelkit`.`code_lang` = 'en')) order by `pvlng_babelkit`.`code_set`,`pvlng_babelkit`.`code_lang`,`pvlng_babelkit`.`code_code` */;
